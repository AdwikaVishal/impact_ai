"""
Decision Maker Finder Module - V2 with Prospeo Integration
Identifies key stakeholders using Prospeo.io (primary) and Hunter.io (fallback)
"""
import requests
import os
...
class DecisionMakerFinder:
    ...
    def find_decision_makers(self, company_name: str, domain: str, category: str) -> List[DecisionMaker]:
        ...
        """
        # Try Prospeo first (better coverage, more data)
        if self.prospeo_key:
    ...
    def _find_via_prospeo(self, company_name: str, domain: str, category: str) -> List[DecisionMaker]:
        """
        Find decision makers using Prospeo API
        Better coverage and more data than Hunter.io
        """
        try:
            # Prospeo company search endpoint
            response = requests.get(
                "https://api.prospeo.io/company-search",
                headers={
                    "X-KEY": self.prospeo_key,
            ...
            if response.status_code != 200:
                print(f"Prospeo API error: {response.status_code}")
                return []
            ...
            for person in data.get("response", {}).get("people", []):
                title = person.get("title", "").lower()
                
                # Check if title matches relevant roles
                if any(role in title for role in relevant_titles):
                    # Calculate relevance score
                    relevance = self._calculate_relevance(person.get("title", ""), category)
            ...
            print(f"Error with Prospeo API: {e}")
            return []
    
    def _find_via_hunter(self, company_name: str, domain: str, category: str) -> List[DecisionMaker]:
        """
        Find decision makers using Hunter.io API (fallback)
        """
        try:
            ...
            print(f"Error finding decision makers via Hunter.io: {e}")
            return []
    ...
    def _fallback_decision_makers(self, company_name: str, category: str) -> List[DecisionMaker]:
        """
        Fallback decision makers when APIs fail
        """
